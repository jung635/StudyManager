...
지각 시작 시간 : <input type="time" name="late_start" value="${gb.late_start }"><br>
	...
	<table border="1">
		<th>아이디</th><th>이름</th><th>나이</th><th>성별</th><th>이메일</th><th>출결 처리</th><th>출결 결과</th><th>출석 시간</th><th>지각비</th><th>결석비</th><th>수정</th>
			<c:forEach items="${group_map }" var="map">
				<tr><td>${map.key.id }</td><td>${map.key.name }</td>
					...
					<td><input type="button" value="수정" onclick="fix('${map.value.num }')"></td>
				</tr>
			</c:forEach>
	</table>
</form>
